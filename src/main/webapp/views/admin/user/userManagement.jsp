            ...
            <body class="bg-background-light dark:bg-background-dark font-display text-slate-800 dark:text-slate-200">
                                                ...
                                                <tr class="teacher-row hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors"
                                                    ...
                                                    <td class="px-6 py-4">
                                                        <c:if test="${t.cvUrl != null && t.cvUrl != ''}">
                                                            <a href="${t.cvUrl}" target="_blank" class="text-primary hover:underline text-sm font-medium flex items-center gap-1">
                                                                <span class="material-icons text-sm">description</span> View
                                                            </a>
                                                        </c:if>
                ...
                <script>
                    ...
                    function filterCurrentView() {
                        if (currentView === 'users') filterGeneric('.user-row', 'visibleRange', 'visibleCount', 'paginationControls', 'noUserFound', 'changePage');
                        else if (currentView === 'teachers') filterGeneric('.teacher-row', 'teacherVisibleRange', 'teacherVisibleCount', 'teacherPaginationControls', 'noTeacherFound', 'changePage');
                        else if (currentView === 'students') filterGeneric('.student-row', 'studentVisibleRange', 'studentVisibleCount', 'studentPaginationControls', 'noStudentFound', 'changePage');
                    }
                    ...
                    function filterGeneric(rowSelector, rangeId, countId, pagId, noFoundId, pageFunc) {
                        ...
                        rows.forEach(row => {
                            ...
                            if (text.includes(query)) filteredRows.push(row);
                        });
                        currentPage = 1;
                    ...
                    function changePage(page) {
                        ...
                        currentPage = page;
                        if (currentView === 'users') renderGeneric('.user-row', 'visibleRange', 'visibleCount', 'paginationControls', 'noUserFound', document.querySelectorAll('.user-row').length);
                        else if (currentView === 'teachers') renderGeneric('.teacher-row', 'teacherVisibleRange', 'teacherVisibleCount', 'teacherPaginationControls', 'noTeacherFound', document.querySelectorAll('.teacher-row').length);
                        else if (currentView === 'students') renderGeneric('.student-row', 'studentVisibleRange', 'studentVisibleCount', 'studentPaginationControls', 'noStudentFound', document.querySelectorAll('.student-row').length);
                    }
                    ...
                    function renderGeneric(rowSelector, rangeId, countId, pagId, noFoundId, totalRowCount) {
                        ...
                        if (totalPages <= 1) { pagContainer.innerHTML = ''; }
                        else {
                            ...
                            if (totalPages > 15) { sp = Math.max(1, currentPage - 5); ep = Math.min(totalPages, currentPage + 5); }
                            for (let i = sp; i <= ep; i++) {
                        ...
                        if (noFound) noFound.style.display = (filteredRows.length === 0 && totalRowCount > 0) ? 'block' : 'none';
                    }
                    ...
                    function openUpdateTeacherModal(id) {
                        ...
                        fetch(contextPath + '/admin/users?action=getTeacherById&id=' + id)
                            ...
                            .catch(err => { console.error(err); alert('Could not load teacher data!'); });
                    }
                    ...
                    function openUpdateStudentModal(id) {
                        ...
                        fetch(contextPath + '/admin/users?action=getStudentById&id=' + id)
                            ...
                            .catch(err => { console.error(err); alert('Could not load student data!'); });
                    }
                    ...
                    document.addEventListener('keydown', function(e) {
                        if (e.key === 'Escape') {
                            ...
                            if (tm && !tm.classList.contains('hidden')) closeUpdateTeacherModal();
                            if (sm && !sm.classList.contains('hidden')) closeUpdateStudentModal();
                        }
                    });
                </script>
            </body>
