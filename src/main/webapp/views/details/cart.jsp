...
<head>
    ...
    <style>
        body { font-family: "Inter", sans-serif; background: #f8fffc; }
                    ...
                    <c:choose>
                        <c:when test="${empty sessionScope.cart or sessionScope.cart.size() == 0}">
                            <div class="flex flex-col items-center justify-center py-24 bg-[#0B1120]/5 rounded-2xl border border-slate-200/50 shadow-sm text-center">
                                ...
                                <a href="${pageContext.request.contextPath}/shop" class="px-8 py-3.5 bg-primary text-white font-bold rounded-xl hover:bg-emerald-600 transition-colors shadow-lg shadow-emerald-500/30">
Explore Courses
                                </a>
                            </div>
                        </c:when>
                            ...
                            <c:forEach var="entry" items="${sessionScope.cart}">
                                ...
                                <div class="flex flex-col sm:flex-row gap-6 p-5 bg-white rounded-2xl border border-emerald-100 shadow-sm hover:shadow-md transition-shadow relative group">
                                    <div class="w-full sm:w-40 aspect-video sm:aspect-square flex-shrink-0 bg-slate-50 rounded-xl overflow-hidden relative">
                                        <img src="${empty course.thumbnailUrl ? pageContext.request.contextPath.concat('/assets/images/default-course.jpg') : course.thumbnailUrl}" alt="course" class="w-full h-full object-cover">
                                    </div>
                                    ...
                                    <div class="absolute bottom-5 right-5 delete-container">
                                        <button type="button" onclick="toggleDeletePopup('popup-${course.courseId}')" class="text-slate-300 hover:text-red-500 transition-colors focus:outline-none">
                                        ...
                                        <div id="popup-${course.courseId}" class="hidden absolute bottom-full right-0 mb-3 w-[300px] bg-white rounded-xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.25)] border border-slate-100 p-4 z-50 transform origin-bottom-right transition-all">
                                            ...
                                            <div class="relative z-10">
                                                ...
                                                <div class="flex justify-end gap-2">
                                                    <button type="button" onclick="toggleDeletePopup('popup-${course.courseId}')" class="px-4 py-1.5 bg-white border border-slate-200 text-slate-600 text-[14px] rounded-lg hover:bg-slate-50 transition-colors focus:outline-none">No</button>
                                                    <a href="${pageContext.request.contextPath}/removeFromCart?id=${course.courseId}" class="px-5 py-1.5 bg-blue-500 text-white text-[14px] rounded-lg hover:bg-blue-600 font-medium transition-colors" style="text-decoration:none;">Yes</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="sticky top-[120px] bg-white p-6 rounded-2xl border border-emerald-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                        ...
                        <div class="mb-6">
                            ...
                            <div class="flex gap-2">
                                <input type="text" placeholder="Enter promo code" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all">
                        ...
                        <div class="flex items-center justify-between mb-8 pb-6 border-b border-slate-100">
                            ...
                            <div class="relative inline-block w-10 mr-2 align-middle select-none transition duration-200 ease-in">
                                <input type="checkbox" name="toggle" id="toggle" class="toggle-checkbox absolute block w-5 h-5 rounded-full bg-white border-4 appearance-none cursor-pointer transition-transform duration-200 ease-in-out z-10 top-0 left-0 border-slate-300"/>
                        ...
                        <button class="w-full flex justify-center items-center gap-2 py-3.5 bg-primary text-white text-[15px] font-bold rounded-xl hover:bg-emerald-600 hover:shadow-lg hover:shadow-primary/30 transition-all duration-300">
            ...
            <c:if test="${not empty suggestedCourses}">
                <div class="mt-20 pt-10 border-t border-slate-100">
                    ...
                    <div class="relative group/slider">
                        ...
                        <div id="suggestedSlider" class="flex gap-6 overflow-x-auto snap-x snap-mandatory pb-6" style="scrollbar-width: none; -ms-overflow-style: none;">
                            <style> #suggestedSlider::-webkit-scrollbar { display: none; } </style>
                            <c:forEach var="sc" items="${suggestedCourses}">
                                <div class="snap-start flex-shrink-0 w-full sm:w-[calc(50%-12px)] lg:w-[calc(25%-18px)]">
                                    <div class="bg-white rounded-2xl border border-emerald-100 overflow-hidden hover:-translate-y-1 hover:shadow-lg transition-all duration-300 flex flex-col group h-full">
                                        <a href="${pageContext.request.contextPath}/course?id=${sc.courseId}" class="aspect-video bg-slate-100 relative overflow-hidden block">
                                            <img class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" src="${empty sc.thumbnailUrl ? pageContext.request.contextPath.concat('/assets/images/default-course.jpg') : sc.thumbnailUrl}" alt="${sc.title}">
                                        </a>
                                        <div class="p-5 flex flex-col flex-1">
                                            <a href="${pageContext.request.contextPath}/course?id=${sc.courseId}" class="font-bold text-[14px] leading-snug mb-2 line-clamp-2 text-slate-900 hover:text-primary transition-colors" style="text-decoration:none;">${sc.title}</a>
                                            <div class="flex items-center text-yellow-400 mb-4">
                                                <span class="text-[12px] text-slate-600 font-bold mr-1">5.0</span>
                                                ...
                                                <div>
                                                    <p class="text-primary font-black text-lg">
                                                        <c:choose>
                                                            ...
                                                            <c:otherwise>$<fmt:formatNumber value="${sc.price}" minFractionDigits="2" maxFractionDigits="2"/></c:otherwise>
                                                        </c:choose>
                                                    </p>
                                                </div>
                                                <a href="${pageContext.request.contextPath}/addToCart?id=${sc.courseId}" class="w-8 h-8 rounded-lg border border-primary text-primary flex items-center justify-center hover:bg-emerald-50 transition-colors" title="Add to Cart">
                        ...
                        <button id="btnSlideRight" class="absolute -right-5 top-1/2 -translate-y-1/2 w-11 h-11 bg-white rounded-full flex items-center justify-center text-slate-400 hover:text-primary shadow-[0_4px_15px_rgba(0,0,0,0.1)] border border-slate-100 z-10 opacity-0 group-hover/slider:opacity-100 transition-all duration-300 transform scale-90 hover:scale-110">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (slider && btnLeft && btnRight) {
                btnRight.addEventListener('click', function(e) { e.preventDefault(); const scrollAmount = slider.children[0].clientWidth + 24; slider.scrollBy({ left: scrollAmount, behavior: 'smooth' }); });
                btnLeft.addEventListener('click', function(e) { e.preventDefault(); const scrollAmount = slider.children[0].clientWidth + 24; slider.scrollBy({ left: -scrollAmount, behavior: 'smooth' }); });
                const handleScrollButtons = () => {
                    btnLeft.style.display = (slider.scrollLeft <= 5) ? 'none' : 'flex';
                    btnRight.style.display = (Math.ceil(slider.scrollLeft + slider.clientWidth) >= slider.scrollWidth - 5) ? 'none' : 'flex';
                };
