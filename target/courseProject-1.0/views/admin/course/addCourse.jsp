...
<div class="relative z-10 w-full max-w-4xl bg-white dark:bg-slate-50 border border-primary/10 shadow-2xl rounded-xl overflow-hidden flex flex-col max-h-[90vh]">
    ...
    <div class="px-8 py-6 border-b border-primary/10 flex items-center justify-between bg-white dark:bg-[#1a2e16] sticky top-0 z-20">
        <div>
            <h2 class="text-2xl font-bold text-slate-900 dark:text-white">Add New Course</h2>
            <p class="text-sm text-slate-500 dark:text-primary/60 mt-1">Fill in the details to create your E-learning content.</p>
        </div>
        <button type="button"
                onclick="closeAddCourseModal()" class="p-2 hover:bg-primary/10 rounded-full transition-colors text-slate-400 dark:text-primary/40 hover:text-primary">
            <span class="material-icons">close</span>
        </button>
    </div>
    <!-- Form Body (Scrollable) -->
    <form id="addCourseForm" action="courseAdmin?action=add" method="post" enctype="multipart/form-data" class="modal-body overflow-y-auto p-8 space-y-8">
            ...
            <div class="lg:col-span-8 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="space-y-2">
                        ...
                        <input type="hidden" name="level" id="levelInput" value="Beginner">
                        ...
                        <div id="levelGroup"
                            ...
                            <button type="button"
                                    data-level="Beginner"
                                    class="add-level-btn flex-1 py-2 text-xs font-semibold rounded-md bg-white dark:bg-primary text-slate-700 dark:text-background-dark shadow-sm transition-all duration-300">
                                Beginner
                            </button>

                            <button type="button"
                                    data-level="Intermediate"
                                    class="add-level-btn flex-1 py-2 text-xs font-semibold text-slate-500 dark:text-primary/60 transition-all duration-300">
                                Intermediate
                            </button>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input required="" name="price" class="w-full pl-8 pr-4 py-3 bg-slate-50 dark:bg-[#11200e] border border-slate-200 dark:border-primary/20 rounded-lg focus:ring-2 focus:ring-primary/50 focus:border-primary outline-none transition-all dark:text-white placeholder:text-slate-400 dark:placeholder:text-primary/30" placeholder="0.00" type="number"/>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="px-8 py-6 border-t border-primary/10 bg-slate-50 dark:bg-[#1a2e16] flex flex-col sm:flex-row items-center justify-end space-y-3 sm:space-y-0 sm:space-x-4">
            <button type="button"
                    onclick="closeAddCourseModal()" class="w-full sm:w-auto px-6 py-2.5 text-sm font-semibold text-slate-600 dark:text-primary/70 hover:text-slate-900 dark:hover:text-primary border border-slate-300 dark:border-primary/20 rounded-lg hover:bg-slate-100 dark:hover:bg-primary/5 transition-all"  >
                Cancel
            </button>
