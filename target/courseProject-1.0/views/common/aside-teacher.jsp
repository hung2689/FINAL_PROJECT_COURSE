<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
    <aside class="w-64 bg-sidebar-dark text-slate-400 flex flex-col fixed h-full z-50">
        <!-- Active Nav Indicator -->
        <div id="nav-indicator"
            class="absolute right-0 w-1 bg-primary rounded-l-full pointer-events-none transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)]"
            style="opacity: 0;"></div>
        <div class="p-6 flex items-center gap-3">
            <div class="w-8 h-8 bg-primary rounded-lg flex items-center justify-center">
                <span class="material-icons text-background-dark text-xl font-bold">school</span>
            </div>
            <span class="text-white font-bold text-xl tracking-tight">DevLearn</span>
            <span class="ml-auto text-[10px] font-bold bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded-full uppercase tracking-wider">Teacher</span>
        </div>
        <nav class="flex-1 px-4 space-y-1 mt-4">
            <a class="flex items-center gap-3 px-4 py-3 rounded-lg ${activeMenu == 'dashboard' ? 'bg-primary/10 text-primary' : 'hover:bg-slate-800'} transition-colors"
                href="${pageContext.request.contextPath}/teacherDashboard">
                <span class="material-icons text-xl">dashboard</span>
                <span class="font-medium">Dashboard</span>
            </a>
            <a class="flex items-center gap-3 px-4 py-3 rounded-lg ${activeMenu == 'assignments' ? 'bg-primary/10 text-primary' : 'hover:bg-slate-800'} transition-colors"
                href="${pageContext.request.contextPath}/teacher/assignments">
                <span class="material-icons text-xl">assignment_turned_in</span>
                <span class="font-medium">Assignment Results</span>
            </a>
        </nav>
        <div class="p-4 border-t border-slate-800">
            <jsp:include page="/views/common/userbuttom.jsp">
                <jsp:param name="isSidebar" value="true" />
            </jsp:include>
        </div>
    </aside>

    <style>
        .teacher-main {
            opacity: 0;
            transform: translateX(12px);
            animation: teacherPageIn 0.3s ease-out forwards;
        }

        @keyframes teacherPageIn {
            to {
                opacity: 1;
                transform: translateX(0);
            }
        }

        .teacher-main.page-leaving {
            animation: teacherPageOut 0.25s ease-in forwards;
        }

        @keyframes teacherPageOut {
            to {
                opacity: 0;
                transform: translateX(-12px);
            }
        }
    </style>
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            var main = document.querySelector('main.flex-1');
            if (main) main.classList.add('teacher-main');

            var aside = document.querySelector('aside');
            if (!aside) return;

            var indicator = document.getElementById('nav-indicator');
            function updateIndicator(link, animate) {
                if (!link || !indicator) return;
                var asideRect = aside.getBoundingClientRect();
                var linkRect = link.getBoundingClientRect();

                if (!animate) {
                    indicator.style.transition = 'none';
                } else {
                    indicator.style.transition = '';
                }

                indicator.style.transform = 'translateY(' + (linkRect.top - asideRect.top) + 'px)';
                indicator.style.height = linkRect.height + 'px';
                indicator.style.opacity = '1';

                if (!animate) {
                    void indicator.offsetWidth;
                    indicator.style.transition = '';
                }
            }

            var activeLink = aside.querySelector('.text-primary');
            if (activeLink) {
                updateIndicator(activeLink, false);
            }

            aside.addEventListener('click', function (e) {
                var link = e.target.closest('a');
                if (!link) return;

                var href = link.getAttribute('href');
                if (!href || href === '#' || href.startsWith('javascript:')) return;
                if (link.target === '_blank') return;
                if (link.hostname && link.hostname !== location.hostname) return;

                e.preventDefault();

                updateIndicator(link, true);

                if (activeLink) {
                    activeLink.classList.remove('bg-primary/10', 'text-primary');
                    activeLink.classList.add('hover:bg-slate-800');
                }
                link.classList.remove('hover:bg-slate-800');
                link.classList.add('bg-primary/10', 'text-primary');
                activeLink = link;

                if (main) {
                    main.classList.add('page-leaving');
                    setTimeout(function () { window.location.href = href; }, 250);
                } else {
                    window.location.href = href;
                }
            });
        });
    </script>
